...
export const allTransactionList = (count: number) => {
    ...
    const html = <View>
        {countList.map((item, index) => (
            <View key={`all-tx-skeleton-${index}`} style={{ ...styles.textSkeleton, width: '100%', height: 55, borderRadius: 10, marginBottom: 6 }} />))}
    </View>;
    return html;
};
export const transactionsCard = () => {
    const html = <View style={{}}>
        <View style={{ ...styles.textSkeleton, width: "100%", height: s(110), marginRight: 6, borderRadius: 10 }} />
...
export const CardActionsSkeleton = (count: number) => {
    let countList = [0];
    if (count) {
        for (let i = 1; i < count; i++) {
            countList.push(i)
    ...
    const html = <View >
        {countList.map((index) => (
          <View  key={`${index}`} style={{ ...styles.textSkeleton, width: '100%', height: s(90), borderRadius: 8 }} />))}
    </View>;
    return html;
};
