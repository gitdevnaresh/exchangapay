...
const WithdrawCrypto = (props: any) => {
  const {
    transactionIdList,
    decimalPlaces,
  ...
  return (
    <ViewComponent>
      ...
      {transactionIdList?.name && renderTransactionField?.("GLOBAL_CONSTANTS.WALLET_NAME", transactionIdList?.name || "")}
      ...
      {renderTransactionField?.("GLOBAL_CONSTANTS.NET_AMOUNT", transactionIdList?.amount || "",
        <CurrencyText
          decimalPlaces={decimalPlaces}
          value={typeof transactionIdList?.amount === 'string' ? parseFloat(transactionIdList.amount.replace(/,/g, '')) || 0 : transactionIdList?.amount || 0}
          style={[commonStyles.listprimarytext, commonStyles.textRight]}
        />
      )}
