...
const SendCryptoSuccess = React.memo((props: any) => {
    ...
    const getHash = async () => {
        setLoading(true)
        try {
            const res: any = await SendCryptoServices.getSendCryptoHash(props.route.params.transactionId);
            if (res?.status === 200) {
                setHash(res.data);
                setLoading(false);
                setErrMsg('');
            ...
            <ScrollView>
                <Container style={commonStyles.container}>
                    ...
                    <View style={[commonStyles.flex1]}>
                        <>
                            <ImageBackground source={require("../../assets/images/cards/Subtract.png")} resizeMode='contain' style={[{ position: "relative", height: 450, }]} >

                                <View style={{ height: 216, marginBottom: 16, marginTop: 16, flexDirection: "row", alignItems: "center" }}>
                                    <View style={[commonStyles.mxAuto, { marginTop: "auto", marginBottom: "auto" }]}>
                                        <Image style={[commonStyles.mxAuto]} source={require('../../assets/images/cards/checkmark.png')} />
                                        <View style={[commonStyles.mb8]} />
                                        <ParagraphComponent text={`Withdraw Submitted`} style={[commonStyles.fs20, commonStyles.fw700, commonStyles.textCenter, { color: NEW_COLOR.TEXT_BLACK }]} />
                                    </View>
                                </View>

                                <View style={[styles.border]} />
                                <View style={[commonStyles.flex1, commonStyles.dflex, commonStyles.justifyCenter, commonStyles.alignCenter, commonStyles.mb20,]}>
                                    <View style={[{ marginTop: "auto", marginBottom: "auto" }]}>
                                        <View>
                                            ...
                                            <TouchableOpacity activeOpacity={0.8} onPress={() => hash && Linking.openURL(hash?.explorer + hash?.transactionHash)} style={{ marginHorizontal: 24 }}>
                                                ...
                                                {!loading && hash ? (
                                                    <View >
                                                        <ParagraphComponent text={hash?.transactionHash} style={[commonStyles.fs10, commonStyles.fw500, commonStyles.textCenter, { color: NEW_COLOR.TEXT_BLACK, marginBottom: 10 }, commonStyles.textCenter]} />
                                                        {hash.transactionHash && <View style={commonStyles.mxAuto}><CopyCard onPress={() => copyToClipboard(hash?.transactionHash)} /></View>}
                                                    </View>
                                                ) : (
                                                    <ParagraphComponent text="" style={[commonStyles.fs12, commonStyles.fw500, commonStyles.textCenter, { color: NEW_COLOR.TEXT_BLACK, marginBottom: 10 }, commonStyles.textCenter]} />
                                                )}
                                            </TouchableOpacity>
                                        </View>
...
const themedStyles = StyleService.create({
    ...
    textDARKGreen: { color: NEW_COLOR.TEXT_DARKGREEN, borderBottomWidth: 1, borderBottomColor: '#D4DADA', borderStyle: 'dashed', paddingBottom: 24, marginBottom: 24, },
    flex1: { flex: 1 },
    mt26: {
        marginTop: 26
    },
    ...
    border: {
        borderTopWidth: 2,
        marginBottom: 10, opacity: 0.2, width: '95%',
