                ...
                <Container style={commonStyles.container}>
                    ...
                    {!commonCryptoLoading && <>
                            ...
                            <View style={[commonStyles.dflex, commonStyles.gap16, styles.flexWrap, commonStyles.justifyCenter, commonStyles.mb8]}>
                                {networkLu?.length > 0 && networkLu?.map((item: any) =>
                                    <TouchableOpacity onPress={() => { handleCryptoDepositCall(item) }} style={[styles.tabStyle, { backgroundColor: network === item?.name ? NEW_COLOR.NETWORK_BLUE : "transparent", }]} activeOpacity={0.9} disabled={network === item?.name ? true : false}>
                                        <ParagraphComponent style={[commonStyles.fs14, commonStyles.fw600, commonStyles.textAlwaysWhite, commonStyles.flex1, commonStyles.textCenter]} text={item?.code} />
                                    </TouchableOpacity>
                                )}
                            </View>
                        ...
                        <ImageBackground resizeMode='contain' style={{ position: "relative", width: "100%", height: isPad ? s(380) : s(350), minHeight: s(350) }} source={require("../../assets/images/cards/light-purplebg.png")}>
                            <View style={[commonStyles.relative,]}>
                                <TouchableOpacity activeOpacity={1} onPress={handleOpenRBSheet} style={[styles.badgePositin]}>
                                    <View style={[commonStyles.dflex, commonStyles.alignCenter, styles.bgorange, styles.w318, commonStyles.mxAuto]}>
                                        <View >
                                            {cryptoDepositeData.logo && <SvgFromUrl
                                                uri={cryptoDepositeData.logo}
                                                width={s(22)}
                                                height={s(22)}
                                            />}
                                            {!cryptoDepositeData.logo && <Image source={cryptoList[props.route?.params?.cryptoCoin]} style={{ width: 30, height: 30, borderRadius: 40 / 2 }} />}
                                        </View>
                                        <View>
                                            <ParagraphComponent style={[commonStyles.textBlack, styles.ml6, commonStyles.fs16, commonStyles.fw600, { marginTop: -3 }]} text={cryptoDepositeData.code || props.route?.params?.cryptoCoin} />
                                        </View>
                                    </View>
                                </TouchableOpacity>
                                <View style={styles.bgQR}>
                                    <View style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.justifyCenter, { height: "auto", minHeight: isPad ? s(236) : s(220), paddingTop: 16 }]}>
                                        <View style={[commonStyles.flex1, commonStyles.dflex, commonStyles.justifyCenter, commonStyles.alignCenter, { marginTop: "auto", marginBottom: "auto" }]}>
                                            <View style={[styles.bgWhite, commonStyles.justifyCenter, commonStyles.dflex,]}>
                                                <QRCode color={NEW_COLOR.TEXT_WHITE} backgroundColor='#fff' value={cryptoDepositeData?.address || 'Sorry network error'} size={s(165)} />
                                            </View>
                                        </View>
                                    </View>
                                    ...
                                    <View style={[commonStyles.dflex, commonStyles.justifyCenter, commonStyles.alignCenter, { marginTop: "auto", marginBottom: "auto", }]}>
                                        <View style={[commonStyles.flex1, commonStyles.px12, commonStyles.mb16, { marginTop: "auto", }]}>
                                            <ParagraphComponent style={[commonStyles.fs12, commonStyles.fw500, commonStyles.textCenter, commonStyles.textGrey,]} text={'Wallet Address'} />
                                            <View style={[]}>
                                                <ParagraphComponent style={[commonStyles.fs12, commonStyles.fw600, commonStyles.textBlack, commonStyles.textCenter, { marginBottom: s(8) }]} text={`${cryptoDepositeData?.address || '--'}`} />
                                                <TouchableOpacity onPress={() => copyToClipboard(cryptoDepositeData?.address)} style={[commonStyles.mxAuto]} activeOpacity={0.8}>
                                                    <View >
                                                        <CopyCard onPress={() => copyToClipboard(cryptoDepositeData?.address)} />
                                                    </View>
                                                </TouchableOpacity>
                                            </View>
                                        </View>
                        ...
                        <View style={[styles.infoCard, commonStyles.dflex, styles.mb32,]}>
                            <Feather name='info' size={s(20)} color={NEW_COLOR.TEXT_BLACK} style={[commonStyles.mt8, commonStyles.mr12]} />
                            <ParagraphComponent style={[commonStyles.fs12, commonStyles.fw400, styles.mr24, styles.pr16, commonStyles.textBlack,]} text={'Please check the transfer address carefully. wrong currency receive address will cause asset loss and the asset cannot be retrieved.'} />
                        </View>
                ...
                {(!commonCryptoLoading && (cryptoDepositeData?.address && popupModelVisible)) && <Overlay overlayStyle={[styles.overlayContent, { width: WINDOW_WIDTH - 30, height: WINDOW_HEIGHT / 4 }]} isVisible={popupModelVisible}>
                    <View style={[commonStyles.alignCenter, commonStyles.gap10, commonStyles.justifyContent, commonStyles.mb43]}>
                        ...
                        <View style={[commonStyles.gap10]}>
                            <ParagraphComponent style={[commonStyles.fs14, commonStyles.fw500, commonStyles.textpara, commonStyles.textCenter]} text={`A fee of ${cryptoDepositeData?.depositMinimumamount || "0"} ${props.route?.params?.cryptoCoin || ""} will be charged for every ${cryptoDepositeData?.network || ""} deposit transaction.`} />
                            <View style={[commonStyles.mb12]} />
...
const themedStyles = StyleService.create({
    ...
    mr24: { marginRight: 24, },
    infoCard: { backgroundColor: NEW_COLOR.BG_BLUE, paddingVertical: 16, paddingHorizontal: 24, borderRadius: 12, marginTop: 16, },
    copyBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 10 },
    mt34: { marginTop: 34 },
    w318: { marginHorizontal: 28, marginBottom: -42, zIndex: 9, paddingHorizontal: 12, },
    badgePositin: { position: "absolute", top: isPad ? s(-14) : s(-18), left: isPad ? "35%" : "33%" },
    bgorange: { backgroundColor: NEW_COLOR.BG_ORANGE, padding: 8, borderRadius: 100, width: (WINDOW_WIDTH * 30) / 100, },
    ...
    border: {
        height: 1, backgroundColor: NEW_COLOR.BORDER_BOTTOM,
        marginBottom: 10, opacity: 0.2, width: '85%', marginRight: "auto", marginLeft: "auto"
    },
    tabStyle: {
        borderWidth: 1, borderRadius: 8, borderColor: NEW_COLOR.NETWORK_BLUE,
