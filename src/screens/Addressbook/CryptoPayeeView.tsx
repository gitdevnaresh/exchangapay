...
const PayeeDetailsView = (props: any) => {
    ...
    const fetchPayeeDetails = async () => {
        setLoading(true);
        try {
            const res: any = await AddressbookService.getPayeeDetails(props.route?.params?.payeeId);
            if (res.status === 200) {
    ...
    const handleProceed = async () => {
        const popupType = (payeeDetails.status?.toLowerCase() !== "inactive") ? "inactive" : "Active";
        setIsLoaders((prev) => ({
            ...prev,
            isBtnLoading: true,
        }))
        try {
            ...
            const res = await AddressbookService.inActivePayee(popupType, payeeDetails.id, body);
            if (res.status === 200) {
        ...
        <SafeAreaView style={[commonStyles.screenBg, commonStyles.flex1]}>
                    ...
                    {!loading &&
                        ...
                        <View style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.gap16, commonStyles.justifyContent]}>
                            <View style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.gap8]}>
                                ...
                                <ParagraphComponent text={"Whitelist Address Details"} style={[commonStyles.fs16, commonStyles.textBlack, commonStyles.fw800]} />
                            </View>

                            {payeeDetails?.status && <Switch
                                trackColor={{ false: "#767577", true: "#F55D52" }}
                                thumbColor={"#f4f3f4"}
                                ios_backgroundColor="#3e3e3e"
                                onValueChange={handleOpenPopup}
                                value={payeeDetails?.status?.toLowerCase() === "active" && true || false}
                            />}
                        </View>
                    }
                    ...
                    {!loading && payeeDetails && (
                        <>
                            <View style={[styles.mt42, styles.mb32]}>
                                <ParagraphComponent text={decryptAES(payeeDetails.favouriteName) || 'Whitelist Address'} style={[commonStyles.fs24, commonStyles.fw600, commonStyles.textCenter, commonStyles.textBlack]} />
                                <ParagraphComponent text="Whitelist Address Name" style={[commonStyles.fs14, commonStyles.fw500, commonStyles.textGrey, styles.px8, commonStyles.textCenter]} />
                            </View>
                            <ImageBackground
                                resizeMode="contain"
                            ...
                            >
                                ...
                                <View style={[commonStyles.dflex, commonStyles.justifyCenter, commonStyles.alignCenter, { marginTop: "auto", marginBottom: "auto" }]}>
                                    <View style={[commonStyles.flex1, commonStyles.px12, commonStyles.mb16, { marginTop: "auto" }]}>
                                        <ParagraphComponent text="Wallet Address" style={[commonStyles.fs12, commonStyles.fw500, commonStyles.textCenter, commonStyles.textGrey,]} />
                                        <View style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.justifyCenter, commonStyles.gap4]}>
                                            <ParagraphComponent text={payeeDetails.walletaddress || '--'} style={[commonStyles.fs12, commonStyles.fw600, commonStyles.textBlack, commonStyles.textCenter, { width: s(270) }]} />
                                            {payeeDetails.walletaddress && (
                                                <CopyCard onPress={() => copyToClipboard(payeeDetails.walletaddress)} iconShow={true} contentShow={false} />
                                            )}
                                        </View>
                                    </View>
                                </View>
                            ...
                            <View style={[styles.detailsCard, commonStyles.mt16]}>
                                <View style={[commonStyles.dflex, commonStyles.justifyContent, commonStyles.alignCenter, commonStyles?.mb10]}>
                                    <ParagraphComponent text="Currency" style={styles.label} />
                                    <ParagraphComponent text={payeeDetails.currency || '--'} style={styles.infoText} />
                                </View>
                                <View style={[commonStyles.dflex, commonStyles.justifyContent, commonStyles.alignCenter, commonStyles?.mb10]}>
                                    <ParagraphComponent text="Network" style={styles.label} />
                                    <ParagraphComponent text={payeeDetails.network || '--'} style={styles.infoText} />
                                </View>

                                {payeeDetails.status && <View style={[commonStyles.dflex, commonStyles.justifyContent, commonStyles.alignCenter, commonStyles?.mb10]}>
                                    <ParagraphComponent text="Status" style={styles.label} />
                                    <View style={[styles.badge, { backgroundColor: payeeDetails.status?.toLowerCase() === "active" ? NEW_COLOR.BG_GREEN : NEW_COLOR.BG_RED }]}>
                                        <ParagraphComponent
                                ...
                                {payeeDetails.reason && <View style={[commonStyles.dflex, commonStyles.justifyContent, commonStyles.alignCenter, commonStyles?.mb10]}>
                                    <ParagraphComponent text="Reason" style={styles.label} />
                                    <ParagraphComponent text={payeeDetails.reason || "--"} style={[commonStyles.textBlack, commonStyles.textRight, { width: s(200) }]} numberOfLines={3} />
                                </View>}
                            </View>
                            {payeeDetails?.status?.toLowerCase() === "active" && <TouchableOpacity
                                style={[styles.shareButton, commonStyles.mt16, commonStyles.mb16]}
                                onPress={onShare}
                            >
                                <AntDesign name="sharealt" size={s(20)} color={NEW_COLOR.TEXT_ALWAYS_WHITE} />
                                <ParagraphComponent text="Share Whitelist Address" style={styles.shareButtonText} />
                            </TouchableOpacity>}
                        </>
                    )}
            ...
            {popupVisible && (
                <CommonPopup
                    ...
                    title={(payeeDetails?.status?.toLowerCase() == "active") && "Inactive Whitelist Address" || "Active Whitelist Address"}
                    content={
                        <>
                            <ParagraphComponent
                                text={payeeDetails?.status?.toLowerCase() === "inactive" && "Do you want to Active the Whitelist Address?" || "Do you want to Inactive the Whitelist Address?"}
                                style={[commonStyles.fs16, commonStyles.textAlwaysWhite]}
                            />
                            <View style={{ marginBottom: s(30) }} />
                            <DefaultButton title="Proceed" loading={isLoaders.isBtnLoading} onPress={handleProceed} style={{ flex: 1, marginLeft: s(8) }} />
                            <View style={{ marginBottom: s(10) }} />
                            <DefaultButton title="Cancel" loading={isLoaders.isCloseLoading} transparent onPress={handleClosePopup} style={{ flex: 1, marginRight: s(8) }} />
                        </>
                    }
                />
            )}
