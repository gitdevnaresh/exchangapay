...
const AddEditPayeeScreen = (props: any) => {
    ...
    const getNetworksList = async (coin?: any, isChange?: any) => {
        try {
            const res: any = await CryptoServices.getCommonCryptoNetworks(coin || props.route?.params?.cryptoCoin);
            if (res.ok) {
    ...
    const getCryptoWallets = async () => {
        ...
        try {
            ...
            if (response?.status === 200) {
                let updatedCoinsList = response?.data?.map((data: any) => ({ ...data, name: data.walletCode }));
                getNetworksList(props?.route?.params?.walletCode || updatedCoinsList[0]?.name);
    ...
    return (
        <SafeAreaView style={[commonStyles.screenBg, commonStyles.flex1]}>
            <ScrollView keyboardShouldPersistTaps="handled" ref={ref}>
                <Container style={commonStyles.container}>
                    <View style={styles.header}>
                        ...
                        <LabelComponent text={'Add Whitelist Address'} style={styles.title} />
                    </View>
                            ...
                            {({ handleSubmit, handleBlur, touched, errors, setFieldValue }) => (
                                <View style={styles.form}>
                                    ...
                                    <Field
                                        ...
                                        onChange={(value: any) => handleChangeCoin(value, setFieldValue)}
                                        error={errors.coin}
                                    ...
                                    <Field
                                        ...
                                        onChange={(value: any) => handleChangeNetwork(value, setFieldValue)}
                                    ...
                                    <View style={styles.spacer} />
                                    <DefaultButton title="Continue" onPress={handleSubmit} loading={btnLoading} />
                                    <View style={styles.buttonSpacer} />
                                    <DefaultButton title="Cancel" transparent onPress={handleGoBack} style={{ flex: 1, marginRight: s(8) }} />
                                </View>
                            )}
                        </Formik>
                    )}
            ...
            {enableScanner && (
                <Modal animationType="slide" transparent={true} visible={enableScanner} onRequestClose={() => setEnableScanner(false)}>
                    <Container style={styles.modalContainer}>
                        <QRCodeScanner
                            onCaptureCode={(data: string) => {
                                const scannedAddress = data.includes(":") ? data.split(":")[1] : data;
                                formikRef.current.setFieldValue('walletAddress', scannedAddress);
