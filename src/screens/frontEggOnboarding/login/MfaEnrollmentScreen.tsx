...
const MfaEnrollmentScreen: React.FC<MfaEnrollmentScreenProps> = ({ route, navigation }) => {
  ...
  const fetchEnrollmentData = async () => {
    ...
    setError('');
    try {
      const result = await FrontEggService.userMFADetail();
      if (result?.ok) {
        // setQrCodeUri(result.barcodeUri);
      } else {
        setError('Could not start MFA enrollment process.');
      }
      setQrLoading(false);
    } catch (e) {
      setError('An error occurred while fetching MFA enrollment data.');
      setQrLoading(false);
    }
