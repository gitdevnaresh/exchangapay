...
const AddShareHolderDetailsForm = (props: any) => {
    ...
    const getPersionalDetails = async () => {
        ...
        try {
            ...
            if (res?.ok) {
                const data = res?.data?.shareholder?.find((item: any) => item?.id === props?.route?.params?.id);
    ...
    const handleSave = async (values: any) => {
        ...
        if (props?.route?.params?.id || props?.route?.params?.addShareHolderDetails) {
            try {
                let response;
                if (props?.route?.params?.addShareHolderDetails) {
                    response = await ProfileService.postShareHoldersDetails(UpdateObj);
                } else {
                    response = await ProfileService.updateShareHoldersDetails(UpdateObj);
                }
                if (response?.ok) {
                    setBtnLoading(false);
                    setErrormsg('');
        ...
        <ViewComponent style={[commonStyles.screenBg, commonStyles.flex1]}>
            ...
            >
                <Container style={commonStyles.container}>
                    <PageHeader title={props?.route?.params?.id ? "GLOBAL_CONSTANTS.EDIT_SHAREHOLDER" : "GLOBAL_CONSTANTS.ADD_SHAREHOLDER"} onBackPress={handleBack} isrefresh={Boolean(props?.route?.params?.id)} onRefresh={handleRefresh} />
...
export default AddShareHolderDetailsForm;
