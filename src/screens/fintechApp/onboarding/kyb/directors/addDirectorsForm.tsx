import { StyleSheet, View, BackHandler, TextInput, Alert } from "react-native";
...
import { KybAddDirectorsSchema } from "../../schema";
...
const KybAddDirectorsDetailsForm = (props: any) => {
    ...
    const uuidv4 = () => {
        return ADD_UBOS_DETAILS_CONSTANTS.GUID_FORMATE.replace(/x/g, function (c) {
            let randumnumberval = Math.random() * 16 | 0;
    ...
    const getPersionalDetails = async () => {
        ...
        try {
            ...
            if (res?.ok) {
                const data = res?.data?.directors?.find((item: any) => item?.id === props?.route?.params?.id);
    ...
    const handleSave = async (values: any) => {
        ...
        if (props?.route?.params?.id || props?.route?.params?.addDirector) {
            try {
                let response;
                if (props?.route?.params?.addDirector) {
                    response = await ProfileService.postDirectorsDetails(UpdateObj);
                } else {
                    response = await ProfileService.updateKybDirectrs(UpdateObj);
                }
                if (response?.ok) {
                    setBtnLoading(false);
                    setErrormsg('');
        ...
        <ViewComponent style={[commonStyles.screenBg, commonStyles.flex1]}>
            ...
            >
                <Container style={commonStyles.container}>
                    <PageHeader title={props?.route?.params?.id ? "GLOBAL_CONSTANTS.EDIT_DIRECTORS_TITLE" : "GLOBAL_CONSTANTS.ADD_DIRECTORS_TITLE"} onBackPress={handleBack} isrefresh={Boolean(props?.route?.params?.id)} onRefresh={handleRefresh} />

                    {
                        <>
                            ...
                            {!loadingData && <Formik
                                ...
                                onSubmit={handleSave}
                                validationSchema={KybAddDirectorsSchema}
                                enableReinitialize
                            >
                                {(formik) => {
                                    const {
...
export default KybAddDirectorsDetailsForm;
