...
export const setFiatChangeText = (text: any, setCryptoConvertVal: Function, setChangeAmt: Function, oneCoinVal: number,  setErrormsg: Function) => {
  setErrormsg("");
  let cleanText = text.replace(/[^0-9.]/g, "").slice(0, 8);
  const decimalIndex = cleanText.indexOf('.');
  if (decimalIndex != -1) {
  ...
  const cryptoVal: number = oneCoinVal ? parseFloat(text || '0') / parseFloat(oneCoinVal.toString()) : 0;
  setChangeAmt((isNaN(cryptoVal) || cryptoVal === 0) ? "" : cryptoVal.toFixed(4));
};