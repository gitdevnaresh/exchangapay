...
import { EmailAuthenticationScreenProps } from './interface';
...
import { getFormattedEmail } from '../../../../utils/helpers';

const EmailAuthenticationScreen: React.FC<EmailAuthenticationScreenProps> = () => {
    ...
    const handleChangeEmail = () => {
        ...
        const actionData: ActionLogParams = {
            screename: 'EmailAuthenticationScreen',
    ...
    const handleBackPress = () => {
        const actionData: ActionLogParams = {
            ...
            actionType: 'Icon',
            nextScreenName: 'LoginVerificationScreen'
        };
    ...
    const handleContinue = async () => {
        ...
        const actionData: ActionLogParams = {
            screename: 'EmailAuthenticationScreen',
            actionName: 'Confirm Change Email',
            actionType: 'Button',
        };
        logEvent('button_press', actionData);
...
const handleCancel=()=>{
    ...
    const EmailAuthenticationeSheetContent = (
        <ViewComponent style={[reversCommonStyles.alignCenter,reversCommonStyles.gap16]}>
            ...
            <TextMultiLanguage
                text={"GLOBAL_CONSTANTS.FOR_SECURITY_REASONS_AFTER_UNBINDING_CHANGING_EMAIL_WITHDRAWALS_AND_INTERNAL_TRANSFERS_WILL_BE_DISABLED"}
        ...
        <SafeAreaView style={[commonStyles.screenBg, commonStyles.flex1]}>
            <ScrollView showsVerticalScrollIndicator={false}>
                <Container style={commonStyles.container}>
                    <PageHeader title="GLOBAL_CONSTANTS.EMAIL_AUTHENTICATION" onBackPress={handleBackPress} />
                    <ViewComponent style={[commonStyles.mt16]} >
                            <ViewComponent style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.gap8]}>
                                <TextMultiLanguage text={"GLOBAL_CONSTANTS.EMAIL"} style={[commonStyles.fs14, commonStyles.textGrey,commonStyles.fw400]} />
                                <ParagraphComponent style={[commonStyles.fs14, commonStyles.textWhite, commonStyles.fw400]} text={getFormattedEmail(decryptAES(userInfo?.email))}/>
                        </ViewComponent>

                        <CommonTouchableOpacity onPress={() => handleChangeEmail()} activeOpacity={0.8} disabled={loading}>
                            <ViewComponent style={[commonStyles.dflex, commonStyles.list, commonStyles.p8, commonStyles.mt24]}>
                                <ViewComponent style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.gap10]}>
                                    <ViewComponent style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.justifyCenter, commonStyles.iconcirclebg]}>
                                        <Ionicons name="mail-outline" size={s(20)} color={NEW_COLOR.TEXT_WHITE} />
                                    </ViewComponent>
                                    <TextMultiLanguage text={"GLOBAL_CONSTANTS.CHANGE_EMAIL_ADDRESS"} style={[commonStyles.fs16, commonStyles.textWhite]} />
                                </ViewComponent>
                                <ViewComponent style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.gap8]}>
            ...
            >
                {EmailAuthenticationeSheetContent}
            </PopupOrSheet>
            {authOpen && <AuthVerification onClose={handleAuthClose} onSuccess={handleAuthSucess} feature={'change Email'}/>}
