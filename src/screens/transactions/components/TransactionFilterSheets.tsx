...
interface TransactionFilterSheetsProps {
  ...
  currencySheetRef: React.RefObject<any>;
...
const TransactionFilterSheets: React.FC<TransactionFilterSheetsProps> = ({
  ...
  const [isConfirmButtonHide, setIsConfirmButtonDisable] = useState<boolean>(true);
  ...
  return (
    <>
      ...
      <PopupOrSheet
        ...
        title={screenName === 'MyCards' ? "GLOBAL_CONSTANTS.TRANSACTION_TYPE" : "GLOBAL_CONSTANTS.CHOOSE_TYPE"}
        titleStyle={[reversCommonStyles.fw700, reversCommonStyles.fs16]}
        height={screenName === 'MyCards' ? s(350) : s(650)}
      >
        <ViewComponent>
          {errorMessage && <ErrorComponent message={errorMessage} />}
          <FlatListComponent
            data={filterOptions.transactionTypeOptions}
            scrollEnabled={true}
            ListEmptyComponent={<NoDataComponent isPopup={true} />}
            keyExtractor={(item: any, index: number) => `${item.value}-${item.recorder || index}`}
            renderItem={({ item }) => {
              const isSelected = filterState.selectedTransactionType === item.value;
              return (
                item.label !== "Card Apply" ? (
                  <CommonTouchableOpacity
                    ...
                    onPress={() => onSelectTransactionType(item.value)}
                  >
                    <ParagraphComponent
                      text={item.label}
                    ...
                    >
                      {isSelected && (<MaterialIcons name="check" size={16} color={"black"} />)}
                    </ViewComponent>
                  </CommonTouchableOpacity>
                ) : null
              );
      ...
      {/* Currency or Cards Bottom Sheet */}
      <PopupOrSheet
        ref={currencySheetRef}
        title={screenName === 'MyCards' ? "GLOBAL_CONSTANTS.SELECT_CARD" : "GLOBAL_CONSTANTS.CURRENCY"}
        height={
          screenName === 'MyCards'
            ? s(350) // smaller height if fewer items
            : s(400) // default height
        }
        showCloseIcon={true}
      >
        <ViewComponent>
          ...
          {filterLoading.currencyLoading ? (
            <ViewComponent style={[reversCommonStyles.alignCenter, reversCommonStyles.p16]}>
              <ParagraphComponent text="Loading..." style={[reversCommonStyles.fs16, { color: REVERSE_NEW_COLOR.TEXT_WHITE }]} />
            </ViewComponent>
          ) : (
            <FlatListComponent
              scrollEnabled={true}
              data={filterOptions.currencyOptions}
              keyExtractor={(item: any) => item.value}
              ListEmptyComponent={<NoDataComponent isPopup={true} />}
              renderItem={({ item }) => {
                const isSelected = filterState.selectedCurrency === item.value;
                const isMyCardsScreen = screenName === 'MyCards';
                return (
                  <CommonTouchableOpacity
                  ...
                  >
                    <ViewComponent style={[reversCommonStyles.dflex, reversCommonStyles.alignCenter]}>
                      {isMyCardsScreen && (
                        <ImageBackgroundWrapper
                          ...
                          resizeMode="cover"
                          imageStyle={[reversCommonStyles.rounded4]}
                          style={[{ height: s(40), width: s(60), marginRight: s(12) }]}
                        >
                          {item?.name?.toLowerCase() === "visa" && <ViewComponent style={[reversCommonStyles.flex1, { justifyContent: 'flex-end', alignItems: 'flex-end', padding: s(3) }]}>
                            <VisaHorizontalImage width={s(16)} height={s(16)} />
                          </ViewComponent>}
                          {item?.name?.toLowerCase() === "master" && <ViewComponent style={[reversCommonStyles.flex1, { justifyContent: 'flex-end', alignItems: 'flex-end', padding: s(3) }]}>
                            <MAsterIcon width={s(16)} height={s(16)} />
                          </ViewComponent>}

                        </ImageBackgroundWrapper>
                      )}
                      {!isMyCardsScreen && item.logo && item.value !== 'All' && (
                        <ViewComponent style={{ marginRight: s(12) }}>
                          <SvgUri width={s(30)} height={s(30)} uri={item.logo} />
                        </ViewComponent>
                      )}
                      <ParagraphComponent
                        text={item.label}
                        style={[reversCommonStyles.fs16, reversCommonStyles.fw500, reversCommonStyles.textWhite]}
                      />
                    </ViewComponent>
                    <ViewComponent
                      style={[
                    ...
                    >
                      {isSelected && (<MaterialIcons name="check" size={s(16)} color={"black"} />)}
                    </ViewComponent>
                  </CommonTouchableOpacity>
                );
              }}
      ...
      >
              ...
              <CommonTouchableOpacity
                ...
                style={[
                  ...
                  reversCommonStyles.justifyCenter,
                  {
                    ...
                    minWidth: s(80),
                    borderWidth: 1,
                    borderColor: isSelected ? REVERSE_NEW_COLOR.BG_YELLOW : 'gray',
                  },
                ]}
              >
                <ParagraphComponent
        ...
        <ViewComponent style={[reversCommonStyles.dflex, reversCommonStyles.justifyContent, reversCommonStyles.mt16, reversCommonStyles.gap16]} >
          <ViewComponent style={reversCommonStyles.flex1}>
            <ButtonComponent title="Reset" onPress={handleReset} solidBackground={true} />
          </ViewComponent>
          <ViewComponent style={reversCommonStyles.flex1}>
            <ButtonComponent
              title="Confirm"
