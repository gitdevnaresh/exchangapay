...
import { StyleSheet, TouchableOpacity, View, ScrollView, SafeAreaView, BackHandler, KeyboardAvoidingView, Dimensions } from "react-native";
...
const ApplyExchangaCard = (props: any) => {
    ...
    const [kycRequiredWhileApplyCard, setkycRequiredWhileApplyCard] = useState<boolean>(false);
    const [cryptoWalletId, setCryptoWalletId] = useState<any>(null);
    const userInfo = useSelector((state: any) => state.UserReducer?.userInfo);
    const personalInfo = useSelector((state: any) => state.UserReducer?.personalInfo);
    ...
    const isCardKYCCompleted = useSelector((state: any) => state.UserReducer?.isCardKycComplete)
    ...
    const getApplyCardDeatilsInfo = async () => {
        ...
        try {
            ...
            const response: any = await CardsModuleService?.getApplyCardsCustomerInfo(cardId);
            if (response?.status === 200) {
                ...
                setkycRequiredWhileApplyCard(response?.data?.setkycRequiredWhileApplyCard)
                ...
                if (props?.route?.params?.kycFormData) {
                    ...
                    setFormData(prevData => ({
                        ...
                        docissueDate: decryptAES(response?.data?.docissueDate) && new Date(decryptAES(response.data?.docissueDate)) || null,
    ...
    const handleRedirectToExchangeCard = async (values?: any) => {
        ...
        let Obj = {
            ...
            annualSalary: formattedValues?.annualSalary ? parseFloat(formattedValues?.annualSalary) : null,
            accountPurpose: formattedValues?.accountPurpose || null,
            expectedMonthlyVolume: formattedValues?.expectedMonthlyVolume ? parseFloat(formattedValues?.expectedMonthlyVolume) : null,
        };
    ...
    return (
        <KeyboardAvoidingView style={[commonStyles.screenBg, commonStyles.flex1]}>
            <SafeAreaView>
                <ScrollView showsVerticalScrollIndicator={false} ref={ref} contentContainerStyle={{ flexGrow: 1 }} >
                    <Container style={commonStyles.container}>
                        ...
                        {!applyCardsLoading &&
                            <View>
                                ...
                                {errormsg && <><ErrorComponent message={errormsg} onClose={() => setErrormsg("")} /><View style={commonStyles.mt8} /></>}
                                <View style={commonStyles.mt8} />
                                <View >
                                    <StepComponent totalSteps={3} currentStep={1} stepContents={stepContents} />
                                </View>
                                {isPad && <View style={[commonStyles.mb24]} />}
                                <View style={[styles.darkBg, commonStyles.p16, commonStyles.mb16]}>
                                    {applyCardsInfo && applyCardsInfo?.map((item: any, index: any) => (<>
                                        <View key={index} style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.justifyContent,]}>
                                            <ParagraphComponent style={[commonStyles.fs12, commonStyles.fw400, commonStyles.textGrey, commonStyles.flex1]} text={`${item?.name}`} />
                                        ...
                                        {index !== applyCardsInfo.length - 1 && <View style={[commonStyles.dashedLine, commonStyles.mt10, commonStyles.mb10, { opacity: 0.5 }]} />}
                                    </>
                                    ))}
                                        ...
                                        {formikProps => {
                                            ...
                                            return (
                                                <>{(isKycRequired && userInfo?.isKYC) && (<>
                                                    ...
                                                    {((isKycRequired && (!isCardKYCCompleted)) && (!userInfo?.isKYC)) &&
                                                        <View>
                                                    ...
                                                    {((isKycRequired && isCardKYCCompleted) && (!userInfo?.isKYC)) &&
                                                        <View style={[commonStyles.bgCard, commonStyles.mt14, commonStyles.rounded16, commonStyles.p16]}>
                                                            ...
                                                            />
                                                            <ParagraphComponent
                                                                style={[commonStyles.textCenter, commonStyles.textBlack, commonStyles.fs18, commonStyles.fw500, commonStyles.mb16, commonStyles.mt10]}
                                                    ...
                                                    <View style={[commonStyles.mb24]} />
                                                    {(!isKycRequired || (isKycRequired && userInfo?.isKYC)) && <DefaultButton
                                                        title={'Next'}
                                                        style={undefined}
                                                        disable={undefined}
                                                        loading={btnLoader}
...
const styles = StyleSheet.create({
    ...
    circle: { borderRadius: 42 / 2, height: 42, width: 42, backgroundColor: NEW_COLOR.USER_ICON_BG },
    pl12: { paddingLeft: 12 },
    mb28: { marginBottom: 28, },
    mr8: { marginRight: 8 },
    mt32: { marginTop: 32 },
    textNormal: {
        fontSize: 16, fontWeight: "500", lineHeight: 19
    },
