...
import { StyleSheet, View, ScrollView, SafeAreaView, Image, TouchableOpacity, TextInput, Text, ActivityIndicator, Modal, Platform, Alert, Keyboard } from "react-native"
...
import { CREATE_KYC_ADDRESS_CONST, FORM_DATA_CONSTANTS, FORM_DATA_LABEL, FORM_DATA_PLACEHOLDER, KycAddressProps, LoadingState, refKycDetailsInterface } from './constant';
...
const KycAddress: React.FC<KycAddressProps> = ({ handleBlur, values, setFieldValue, handleChange, touched, errors, kycReqList, formData, disableFields, cardId }) => {
    ...
    const handleSaveSignature = (event: any) => {
        ...
        if (pathName || encoded) {
            handleDrawSign(event, FORM_DATA_CONSTANTS?.SIGNATURE)
            togglePopup();
        }
    };
    ...
    const togglePopup = () => {
        setLoadingState((prev: any) => ({ ...prev, signModelVisible: !loadingState?.signModelVisible }))
    ...
    const fetchLookUps = async () => {
        try {
            ...
            if (response.data.IdTypes.length === 1) {
                setFieldValue(FORM_DATA_CONSTANTS?.ID_TYPE, response.data.IdTypes[0].name);
            }
            setErrormsg('');
        } catch (error) {
            ref?.current?.scrollTo({ y: 0, animated: true });
    ...
    const getCardTowns = async (countyCode: any) => {
        ...
        try {
            const response: any = await CardsModuleService.getTownsLookup(cardId, countyCode);
            if (response?.ok) {
    ...
    const getListOfCountryDetails = async () => {
        ...
        if (response?.ok) {
            ...
            if (countryName && response?.data.length >= 1) {
                const _country = response.data?.find((item: any) => item?.name === values?.country);
    ...
    const handlePopup = () => {
        setLoadingState((prev) => ({ ...prev, drawSignModel: !loadingState?.drawSignModel }))
    };

    const selectImage = async (fieldName: string, isCamera?: boolean) => {
        Keyboard.dismiss();
        setLoadingState((prev) => ({ ...prev, drawSignModel: false, facePopup: false }))
        ...
        try {
            ...
            if (isCamera === true) {
                ...
                result = await launchImageLibrary({ mediaType: FORM_DATA_CONSTANTS?.PHOTO });
    ...
    const handleOpenFacePopup = () => {
        setLoadingState((prev) => ({ ...prev, facePopup: !loadingState?.facePopup }))
    };
    ...
    const kycRequirementsDetails = {
        fullname: (<View>
            ...
            <LabelComponent text={FORM_DATA_LABEL.GENDER} Children={<LabelComponent
                text=" *"
                style={commonStyles.textError}
            ...
            <View style={[commonStyles.mb16]} />
            <LabelComponent text={FORM_DATA_LABEL.DATE_OF_BIRTH} Children={<LabelComponent
                text=" *"
                style={commonStyles.textError}
            />} style={[commonStyles.mb10]} />
            <View
                style={[
                    styles.input,
                    {
                        ...
                        alignItems: "center",
                    }, isFieldDisabled(disableFields?.dob, userInfo?.isKYC) ? commonStyles?.disabledBg : null]}>
                {values.dob ? (
                    <ParagraphComponent style={[commonStyles.fs14, commonStyles.textBlack]} text={formatDateMonth(values?.dob)} />
                ) : (
        ...
        comms: (<View>
            ...
            <View style={[commonStyles.relative, commonStyles.dflex, commonStyles.gap8]}>
                <PhoneCodePicker
                    ...
                    containerStyle={[isFieldDisabled(disableFields?.mobileCode) ? commonStyles?.disabledBg : null]}
                    onChange={(item: any) =>
                        setFieldValue("mobileCode", item?.code)
                    }
                ...
                />
                <TextInput
                    style={[styles.inputStyle, commonStyles.flex1, isFieldDisabled(disableFields?.mobile) ? commonStyles?.disabledBg : null]}
        ...
        passport: (<View>
            ...
            <View style={[commonStyles.sectionStyle, commonStyles.flex1]}>
                <View
                    style={[
                        commonStyles.dflex,
                        commonStyles.gap8,
                        commonStyles.mb8, commonStyles.flex1
                    ]}
                >
                    <Feather
                        name="info"
            ...
            <LabelComponent text={FORM_DATA_LABEL.DOCUMENT_EXPIRY_DATE} Children={<LabelComponent
                text=" *"
                style={commonStyles.textError}
            />} style={[commonStyles.mb10]} />
            <View
                style={[styles.input, commonStyles.justifyContent, commonStyles.alignCenter]}>

                {values.docExpiryDate ? (
                    <ParagraphComponent style={[commonStyles.fs14, commonStyles.textBlack]} text={formatDateMonth(values.docExpiryDate)} />
                ) : (
                    <ParagraphComponent style={[commonStyles.fs14, commonStyles.textGrey]} text={PROFILE_CONSTANTS?.DD_MM_YYYY} />
                )}
                {expiryDatePicker && (
                    ...
                    <View style={[commonStyles.flex1]}>
                        <ParagraphComponent
                            style={[
                                ...
                                commonStyles.textGrey,
                                commonStyles.fw400,
                                commonStyles.mb4, commonStyles.flex1
                            ]}
                            text={FORM_DATA_PLACEHOLDER.UPLOAD_NOTE1}
                        />
                        <ParagraphComponent
                            style={[
                                commonStyles.fs12,
                                commonStyles.textGrey,
                                commonStyles.fw400, commonStyles.flex1
                            ]}
                            text={FORM_DATA_PLACEHOLDER.UPLOAD_NOTE2}
                        />
                    </View>
            ...
            <>
                ...
                >
                    <View >
                        ...
                        {uploadValidation?.handHoldingIDPhoto && <Text style={{ color: 'red' }}>{uploadValidation?.handHoldingIDPhoto}</Text>}
        ...
        fulladdress: (<View><View style={[commonStyles.mb8]} />
            <Field
        ...
        financialprofile: (
            <View style={[commonStyles.mt16]}>
                ...
                <Field
                    ...
                    component={InputDefault}
                    onChangeText={(value: any) => handleChangeAnnualSalary(value, setFieldValue)}
                ...
                <Field
                    ...
                    component={InputDefault}
                    maxLength={10}
                    onChangeText={(value: any) => handleChangeExpectedMonthlyVolume(value, setFieldValue)}
    ...
    const handleChangeAnnualSalary = (text: string, setFieldValue: (field: string, value: any) => void) => {
    ...
    return (
        <SafeAreaView>
            <ScrollView ref={ref} keyboardShouldPersistTaps="handled"><View>
                ...
                <View>
                    {kycReqList?.map((kycKey) => {
                        ...
                        return (
                            <View key={kycKey}>
                                {label && (<LabelComponent text={`${label}`} style={[commonStyles.textAlwaysWhite, commonStyles.fs16, commonStyles.mt16]} />
                                )}
            ...
            <Modal
                animationType={FORM_DATA_CONSTANTS?.SLIDE}
                transparent={true}
                visible={loadingState?.signModelVisible}
            ...
            >
                <View style={styles.popupContainer}>
                    <View style={styles.popupContent}>
                        ...
                        >
                            <ParagraphComponent
                                text={FORM_DATA_CONSTANTS?.SIGN_HERE}
                                style={[
                                    commonStyles.textBlack,
                                    commonStyles.fs16,
                                    commonStyles.fw700,
                            ...
                            >
                                <AntDesign
                                    name={FORM_DATA_CONSTANTS?.CLOSE}
                                    size={22}
                                    color={NEW_COLOR.TEXT_BLACK}
                                />
                            </TouchableOpacity>
                ...
                `}
                        ...
                        <DefaultButton
                            title={FORM_DATA_CONSTANTS?.SAVE}
                            style={undefined}
                            disable={undefined}
                            onPress={saveSign}
                        />
                        <View style={[commonStyles.mb16]} />
                        <DefaultButton
                            title={FORM_DATA_CONSTANTS?.RESET}
                            style={undefined}
