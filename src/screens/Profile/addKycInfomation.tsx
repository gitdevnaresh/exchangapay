...
const AddKycInfomation = (props: any) => {
  ...
  const fetchProfileEditView = async () => {
    try {
      ...
      if (response?.ok) {
        ...
        const decryptedExpirationDate = decryptAES(
        ...
        const decryptedDocumentNumber = response.data?.documentNumber
          ...
          : "";
        const decryptedEmergencyContactName = response.data
          ?.emergencyContactName
  ...
  const confirmSelfieUpload = async () => {
    if (!selfiePreview || isSelfieLoading) return;

    const file = selfiePreview;
                      ...
                      <View>
                        <Field
                          ...
                          onChangeText={(value: any) => {
                            handleDocNumber(value, setFieldValue);
                          }}
                          handleBlur={handleBlur}
                          customContainerStyle={{ height: 80 }}
                          placeholder={
