...
const Dashboard = (props: any) => {
  ...
  const noiffCount = useSelector(
    (state: any) => state.UserReducer?.notificationCount
  );
  const supportMessageCount = useSelector(
    (state: any) => state.UserReducer?.supportMessgaeCount
  );
  ...
  const [state, setState] = useState<any>({
    ...
    routes: [
      {
        key: DASHBOARD_CONSTANTS?.FIRST,
        title: DASHBOARD_CONSTANTS?.HOME,
        icon: DASHBOARD_CONSTANTS?.ICON_HOME,
        component: Crypto,
      },
      {
        key: DASHBOARD_CONSTANTS?.SECOND,
        title: DASHBOARD_CONSTANTS?.CARDS,
        icon: DASHBOARD_CONSTANTS?.EXCHANGE,
        component: NewCard,
      },
      {
        key: DASHBOARD_CONSTANTS?.THIRD,
        title: DASHBOARD_CONSTANTS?.TRANSACTIONS,
        icon: DASHBOARD_CONSTANTS?.TRANSACTION,
        component: CryptoCardsTransaction,
      },
    ],
  });
  const [isAccountpopupVisible, setIsAccountPopupVisible] =
  ...
  useEffect(() => {
    if (
      ...
      props.route.params.tabIndex !== state.index
    ) {
  ...
  useEffect(() => {
    if (!props?.route?.params?.isUserUpdate && userInfo) {
      const isFirstNameEmpty =
        ...
        decryptAES(userInfo.firstName) === null;
      if (isFirstNameEmpty) {
    ...
    <>
        ...
        <View style={[commonStyles.screenBg, commonStyles.px16]}>
          {state.index === 0 && (
            <>
              <View
                style={[
                  userInfo?.accountStatus === DASHBOARD_CONSTANTS?.INACTIVE &&
                  Platform.OS === "android"
                    ? commonStyles.mb10
                    : commonStyles.mb32,
                ]}
                ...
                accessoryLeft={(): TouchableWebElement => (
                  <View>
                    ...
                    >
                      ...
                      <ParagraphComponent
                        text={DASHBOARD_CONSTANTS?.EXCHANGA_PAY}
                        style={[
                          commonStyles.fs22,
                          commonStyles.fw700,
      ...
      <Tab.Navigator
        screenOptions={({ route }) => ({
          ...
          tabBarItemStyle: {
            paddingVertical: 4,
            borderEndWidth: route.name !== "Transactions" ? 0.7 : 0,
            borderEndColor:
      ...
      >
        ...
        <Tab.Screen
          name={DASHBOARD_CONSTANTS?.TRANSACTIONS}
          component={CryptoCardsTransaction}
        />
      </Tab.Navigator>
