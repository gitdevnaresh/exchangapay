    ...
    const handleRefresh = async () => {
        ...
        try {
            ...
            if (userLoginInfo?.status === 200) {
                setBtnLoader(false);
                userDetails?.isEmailVerified ? getMemDetails({}) : setPopupVisible(true);
            } else {
                setBtnLoader(false);
                setErrorMsg(isErrorDispaly(userLoginInfo));
            }
        } catch (error) {
            setBtnLoader(false);
    ...
    return (
        <Container style={commonStyles.container}>
            <ScrollView>
                ...
                <View style={[commonStyles.mt30]}>
                    ...
                    <Text style={[styles.textSuccess]}>
                        {EMAIL_CONSTANTS.VERIFY_YOUR_EMAIL}
                    </Text>
                        ...
                        {!loadMail &&
                            <View style={[commonStyles.dflex, commonStyles.alignCenter, commonStyles.justifyCenter,]}>
                                <TouchableOpacity onPress={resendMail} >
                                    <Text style={[styles.resend, commonStyles.textOrange, commonStyles.fs16, commonStyles.fw600]}>{EMAIL_CONSTANTS.RESEND}</Text>
                                </TouchableOpacity>
                            </View>
                        }
                    </View>
            ...
            <CommonPopup
                ...
                buttonName="Continue"
                onButtonPress={() => setPopupVisible(false)}
            />
