...
const CardBalance = React.memo((props: any) => {
  ...
  const [feeComissionLoading, setFeeComissionLoading] =
  ...
  const [networkModelVisible, setNetworkModelVisible] =
    useState<boolean>(false);
  ...
  const getExchangeDeposite = async (walletCode?: any) => {
    ...
    try {
      const response: any = await CardsModuleService.getDepositData(
        cardId,
        walletCode
      );
      if (response?.status === 200) {
  ...
  const fetchDepositFeeComission = async () => {
    ...
    if (
      ...
      parseFloat(depositData?.depositCryptoMinAmount?.toFixed(2))
    ) {
      setFeeComissionLoading(true);
      try {
        const response: any = await CardsModuleService.getDepositFeeComission(
          topupAmount || 0,
          cardId
        );
        if (response?.status === 200) {
  ...
  const saveTopUp = async () => {
    ...
    if (
      ...
      (topupAmount &&
        !(
          parseFloat(topupAmount) >=
          depositData?.depositCryptoMinAmount?.toFixed(2)
        ))
    ) {
      setTopupLoading(false);
      ref?.current?.scrollTo(0, 0, true);
    ...
    if (
      ...
      (topupAmount &&
        !(
          parseFloat(topupAmount) <=
          depositData?.depositCryptoMaxAmount.toFixed(2)
        ))
    ) {
      setTopupLoading(false);
      ref?.current?.scrollTo(0, 0, true);
    ...
    try {
      ...
      if (res.status === 200) {
        setTopupLoading(false);
        props.navigation.navigate(CONSTANTS.DEPOSIT_SUBMITTED, {
          cardId: props.route.params.cardId,
          reciveAmount: `${formatCurrency(
            feeComissionData?.toTalAmount || 0,
            2
  ...
  const selectedNetworkData = networkList.find(
    (network) => network.code === selectedNetwork
  );
  ...
  return (
    <SafeAreaView style={[commonStyles.screenBg, commonStyles.flex1]}>
            ...
            <View>
              {depositDataLoading ? (
                ...
                <>
                  ...
                  <View
                    style={[
                      commonStyles.orangeSection,
                      commonStyles.dflex,
                      commonStyles.rounded20,

                      {
                        minHeight: isPad ? 150 : 115,
                        paddingVertical: 16,
                        zIndex: 1000,
                      },
                    ]}
                  >
                    <Image
                      style={[styles.hand]}
                      source={require("../../assets/images/cards/deposithand.png")}
                    />
                    <View
                      style={[
                        commonStyles.flex1,
                    ...
                    >
                      <View>
                        ...
                        >
                          <ParagraphComponent
                            text={selectedCoin || depositData?.cryptoCurrency}
                            style={[
                              commonStyles.fs24,
                              commonStyles.textBlack,
                              commonStyles.fw700,
                      ...
                      <ParagraphComponent
                        text={`${formatCurrency(amount)} ${
                          selectedCoin || depositData?.cryptoCurrency
                        }`}
                        style={[
                          commonStyles.fs24,
                          commonStyles.textBlack,
                  ...
                  <View>
                    ...
                    >
                      <ParagraphComponent
                        ...
                        style={[
                          ...
                          commonStyles.textGrey,
                          commonStyles.fw600,
                          { color: NEW_COLOR.INPUT_INSIDE_LABEL },
                        ]}
                      />
                      <View style={[commonStyles.flex1]}>
                        <TextInputField
                          inputStyle={[
                            isPad ? commonStyles.fs24 : commonStyles.fs36,
                        ...
                        <View>
                          <ParagraphComponent
                            text={`${depositData?.cryptoCurrency}`}
                            style={[
                              commonStyles.fs14,
                              commonStyles.textGrey,
                              commonStyles.fw600,
                    ...
                    <ParagraphComponent
                      text={`${CONSTANTS.MAXIMUM_DEPOSIT_AMOUNT_IS}`}
                      style={[
                        commonStyles.fs10,
                        commonStyles.textGrey,
                        commonStyles.fw500,
                      ...
                      children={
                        <ParagraphComponent
                          text={` ${formatCurrency(
                            depositData.depositCryptoMaxAmount || 0,
                            2
                          )} `}
                          style={[
                            commonStyles.fs12,
                          ...
                          children={
                            <ParagraphComponent
                              ...
                              children={
                                <ParagraphComponent
                                  text={` / ${formatCurrency(
                                    depositData?.depositMaxAmount || 0,
                                    2
                                  )} `}
                                  style={[
                                    commonStyles.fs10,
                    ...
                    />
                    <ParagraphComponent
                      text={`${CONSTANTS.MINIMUM_DEPOSIT_AMOUNT_IS} `}
                      style={[
                        commonStyles.fs10,
                        commonStyles.textGrey,
                        commonStyles.fw500,
                      ...
                      children={
                        <ParagraphComponent
                          text={` ${formatCurrency(
                            depositData.depositCryptoMinAmount || 0,
                            2
                          )} `}
                          style={[
                            commonStyles.fs12,
                          ...
                          children={
                            <ParagraphComponent
                              ...
                              children={
                                <ParagraphComponent
                                  text={` /  ${formatCurrency(
                                    depositData?.depositMinAmount || 0,
                                    2
                                  )} `}
                                  style={[
                                    commonStyles.fs10,
                    ...
                    <View style={[commonStyles.sectionStyle]}>
                      ...
                      >
                        <ParagraphComponent
                          ...
                          text={CONSTANTS.ESTIMATED_CRYPTO_AMOUNT}
                        />
                        <ParagraphComponent
                          style={[
                            commonStyles.fs14,
                          ...
                          text={`${formatCurrency(
                            feeComissionData?.estimatedAmount || 0,
                            2
                          )} ${depositData?.cryptoCurrency}`}
                        />
                      </View>
                      ...
                      >
                        <ParagraphComponent
                          ...
                          text={CONSTANTS.TOTAL_RECEIVE_CURRENCY_AMOUNT}
                        />
                        <ParagraphComponent
                          style={[
                            commonStyles.fs14,
                  ...
                  <View>
                    <ParagraphComponent
                      ...
                      text={
                        CONSTANTS.DUE_TO_CURRENCY_PRICE_FLUCTUATIONS_THERE_MAY_BE_A_SMALL_DIFFERENCE_BETWEEN_THE_FINAL_DEDUCTION_AMOUNT_AND_THE_DISPLAYED_AMOUNT
                      }
                    />
                  </View>
                  <View style={[commonStyles.mb43]} />
      ...
      {coinsModelVisible && (
        <CoinsDropdown
          coinsList={coinsDataList}
          modelvisible={() => {
            setCoinsModelVisible(false);
          }}
          handleSelect={handleSelectedCoin}
          label={CONSTANTS.COIN}
          selected={selectedCoin}
        />
      )}
      {networkModelVisible && (
        <CoinsDropdown
          coinsList={networkList}
          modelvisible={() => {
            setNetworkModelVisible(false);
          }}
          handleSelect={handleSelectedNetwork}
          label={CONSTANTS.NETWORK}
...
const themedStyles = StyleService.create({
  ...
  dashedHr: {
    ...
    borderBottomColor: NEW_COLOR.TEXT_BLACK,
    height: 1,
    width: "100%",
    borderStyle: "dashed",
  },
