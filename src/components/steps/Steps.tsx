...
import { MenuIcon, ReviewedIcon } from '../../assets/svg';

const { width } = Dimensions.get('window');
const isPad = width > 600;

const StepComponent = ({ totalSteps, currentStep, stepContents }: any) => {
  const renderSteps = () => {
    const steps = [];
      ...
      const stepContent = isCurrentStep ? (
        <View>
          <View style={{ backgroundColor: '#FFF', borderWidth: 1, borderColor: '#000', height: 36, width: 36, borderRadius: 100 / 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
            {currentStep === 1 ? <MenuIcon /> : currentStep === 2 ? <FontAwesome name='usd' size={18} color={NEW_COLOR.TEXT_GREY} /> : <ReviewedIcon />}
          </View>
          <View style={{ position: "absolute", bottom: ((isPad && currentStep === 3) ? -35 : (isPad && currentStep === 1) ? -50 : (isPad && currentStep === 2) ? -30 : (currentStep === 2 ? -20 : -36)), minWidth: ((isPad && currentStep === 1) ? 180 : (isPad && currentStep === 3 ? 180 : 90)), left: ((isPad && currentStep === 1) ? -60 : (isPad && currentStep === 3) ? -80 : -30), flexDirection: "row", justifyContent: "center", minHeight: (isPad ? 30 : 10) }}>
        ...
        <View>
          <View style={{ backgroundColor: NEW_COLOR.BG_PURPLERDARK, borderWidth: 1, borderColor: NEW_COLOR.BG_PURPLERDARK, height: 36, width: 36, borderRadius: 100 / 2, alignItems: 'center', justifyContent: 'center', flexDirection: "row" }}>
            {isSecondStep && <FontAwesome name='usd' size={18} color={NEW_COLOR.TEXT_GREY} />}

            {isThirdStep && <ReviewedIcon />}
