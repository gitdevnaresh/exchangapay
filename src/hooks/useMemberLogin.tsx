...
import { isLogin, isSessionExpired, setUserInfo } from "../redux/Actions/UserActions";
...
const useMemberLogin = () => {
    ...
    const getMemDetails = async (userInfo?: GetMemDetailsParams, IsCheckBio?: boolean, isOnlyMember?: boolean) => {
        try {
            setLoading(true);
            const userLoginInfo: any = await AuthService.getMemberInfo();
            const userDetails = userLoginInfo?.data;
            if (userLoginInfo?.status === 200) {
                await Keychain.setGenericPassword("userInfo", JSON.stringify(userDetails), {
                ...
                else if (!userDetails?.isPhoneNumberVerified && userDetails?.isPhoneNumberverfiyWhileSignup) {
                    return navigation.dispatch(
                        CommonActions.reset({
                            index: 1,
                ...
                else if ((!userDetails?.isCustomerReferralCode && userDetails?.isReferralRequiredOrNot)) {
                    navigation.dispatch(
                        CommonActions.reset({
                            index: 1,
                            routes: [{
                                name: "rigistrationreferral", params: {
                ...
                } else if (((userDetails?.customerKycRequiredorNot && (!userDetails?.isKYC)) && userDetails.isSumsubKyc)) {
                    if (currentRouteName !== "sumsubCompnent" && currentRouteName !== "underReview") {
                        return navigation.dispatch(
                            CommonActions.reset({
                                index: 1,
                ...
                } else if ((userDetails?.isKYC && (!userDetails?.isSumsubKyc)) && (userDetails.customerState !== "Approved")) {
