...
# M-01: declared false rather than left absent. Environment.js reads
# `=== "true"`, so an absent key and an explicit false behave identically —
# but only one of them says whether that is the intent or an oversight.
# Flip to true ONLY after the backend verifies X-Device-Attestation and rejects
# a bad verdict on every API host (security/ATTESTATION_BACKEND_CONTRACT.md).
# tst is where that flip should be exercised first, ahead of prod.
ATTESTATION_ENFORCEMENT_ENABLED=false
# Relative path on each first-party API host — never a third-party URL. Same
# value as the fallback in Environment.js, declared here so the challenge
# endpoint is a per-environment setting rather than a constant in code.
ATTESTATION_CHALLENGE_PATH=api/v1/security/attestation/challenge
